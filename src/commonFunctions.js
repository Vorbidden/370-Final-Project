...
async function asyncCalcCentroid(vertices) {
...
function parseSceneFile(file, state) {
  return new Promise((resolve, reject) => {
    ...
    fetch(file)
      ...
      .then((sceneText) => {
        const jData = JSON.parse(sceneText);
        state.loadObjects = jData.objects;
...
function parseUIFile(file, state) {
  return new Promise((resolve, reject) => {
    state.uiObjects = [];
    state.loadUIObjects = [];
...
function addUIElement(object, state, vertShader = null, fragShader = null) {
  ...
  tempObject.vertShader = vertShader ? vertShader : state.vertShaderUI;
  tempObject.fragShader = fragShader ? fragShader : state.fragShaderUI;

  tempObject.setup();
  addObjectToUI(state, tempObject);
  return tempObject;
}

async function addMesh(object, vertShader = null, fragShader = null) {
  if (state.meshCache[object.fileName]) { // a way to load the mesh faster by re-using model data
    const created = await createMesh(state.meshCache[object.fileName], object, vertShader, fragShader);
    return created;
  } else {
    const mesh = await parseOBJFileToJSON(object.fileName);
    state.meshCache[object.fileName] = mesh;
